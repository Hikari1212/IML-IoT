...
export function initAdminPage(auth, db, functions, XLSX, Chart) {
    ...
    function setupNavigation() {
        ...
        const navLinks = {
            ...
            'nav-link-admins': document.getElementById('admin-management-panel'),
            'nav-link-logs': document.getElementById('activity-log-panel'),
            'nav-link-analytics': document.getElementById('analytics-panel'),
        ...
        allFeaturesPanel.addEventListener('click', (e) => {
            ...
            if (card && card.dataset.panel) {
                const panelToShow = document.getElementById(card.dataset.panel);
                if (panelToShow) showPanel(panelToShow);
            }
        });
    }
    ...
    function initDiscordManagement() {
        ...
        discordSettingsForm.addEventListener('submit', async (e) => {
            ...
            document.querySelectorAll('.discord-rule').forEach(ruleDiv => {
                updatedRules.push({
                    id: ruleDiv.dataset.id,
                    enabled: ruleDiv.querySelector('.rule-enabled').checked,
                    property: ruleDiv.querySelector('.rule-property').value,
                    value: ruleDiv.querySelector('.rule-value').value,
                    roleId: ruleDiv.querySelector('.rule-role-id').value,
                });
            });
    ...
    function renderDiscordRules() {
        const discordRulesContainer = document.getElementById('discord-rules-container');
        if (!discordRulesContainer) return;
        discordRulesContainer.innerHTML = '';
        discordRules.forEach(rule => {
            const ruleDiv = document.createElement('div');
            ruleDiv.className = 'discord-rule';
            ruleDiv.dataset.id = rule.id;
            const propertyOptions = `
                <option value="status" ${rule.property === 'status' ? 'selected' : ''}>在室状況</option>
                <option value="grade" ${rule.property === 'grade' ? 'selected' : ''}>学年</option>
                <option value="roles" ${rule.property === 'roles' ? 'selected' : ''}>役職</option>
                <option value="isExpired" ${rule.property === 'isExpired' ? 'selected' : ''}>失効状態</option>`;
            ruleDiv.innerHTML = `
                <div class="toggle-switch">
                    <input type="checkbox" id="toggle-${rule.id}" class="rule-enabled" ${rule.enabled ? 'checked' : ''}>
                    <label for="toggle-${rule.id}" class="slider"></label>
                </div>
                <div class="rule-condition">
                    もし <select class="rule-property">${propertyOptions}</select> が
                    <input type="text" class="rule-value" value="${rule.value || ''}" placeholder="例: in, B1, 部長, false"> ならば
                </div>
                <div class="rule-action">
                    ロール <input type="text" class="rule-role-id" value="${rule.roleId || ''}" placeholder="ロールID" pattern="^\\d{17,19}$" required> を付与する
                </div>
        ...
        document.querySelectorAll('.delete-rule-button').forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                discordRules = discordRules.filter(r => r.id !== ruleId);
                renderDiscordRules();
                isDiscordFormDirty = true;
            });
        });
    ...
    // --- 生体情報管理機能 ---
    function initBiometricManagement() {
        ...
        enrollTabButton.addEventListener('click', () => switchBiometricTab(enrollTabButton, enrollModeDiv));
        ...
        const handleBiometricRegistration = async (biometricType) => {
            ...
            try {
                ...
                document.getElementById('token-display-text').textContent = result.data.token;
...
function initFaceEnrollmentTab() {
    ...
    function initApiManagement() {
        ...
        updateApiKeyButton.addEventListener('click', async () => {
            if (!confirm('新しい「部員名簿APIキー」を生成しますか？古いキーは使用できなくなります。')) return;
            const newApiKey = generateRandomString();
            try {
                const updateApiKey = httpsCallable(functions, 'updateApiKey');
                await updateApiKey({ apiKey: newApiKey, keyType: 'memberApiKey' });
        ...
        copyApiUrlButton.addEventListener('click', () => copyToClipboard('api-endpoint-url', copyApiUrlButton));
        copyFaceVerifyApiUrlButton.addEventListener('click', () => copyToClipboard('face-verify-api-endpoint-url', copyFaceVerifyApiUrlButton));
    ...
    function initMemberManagement() {
        ...
        addMemberForm.addEventListener('submit', async (e) => {
            ...
            const expiryChoice = newMemberExpirySelect.value;
            let expiryDate;
    
            if (expiryChoice === 'custom') {
                if (!newMemberCustomExpiryInput.value) {
                    alert('有効期限の年月日を指定してください。');
                    return;
                }
                expiryDate = new Date(newMemberCustomExpiryInput.value);
            } else {
            ...
            const selectedRoles = Array.from(document.querySelectorAll('input[name="new-member-role"]:checked'))
                .map(cb => cb.value);
            ...
            addMemberForm.reset();
            newMemberCustomExpiryInput.style.display = 'none';
            alert(`${nameInput.value} さんを追加しました (キー: ${assignedKey})`);
        });

        adminMemberListDiv.addEventListener('click', async (e) => {
            ...
            if (target.classList.contains('save-button')) {
                ...
                const selectedRoles = Array.from(document.querySelectorAll(`input[name="edit-role-${id}"]:checked`))
                    .map(cb => cb.value);
                const updatedData = {
            ...
            if (target.classList.contains('delete-button') && target.dataset.id) {
        ...
        bulkApplyButton.addEventListener('click', async () => {
            ...
            if (action === 'delete') {
                ...
                if (!choice || !confirm(`選択した ${selectedIds.length} 件の部員の有効期限を更新しますか？`)) return;
                if (choice === 'expire-now') {
                    selectedIds.forEach(id => batch.update(doc(db, 'members', id), { isExpired: true, expiryDate: null }));
                } else {
                    const newExpiryDate = (choice === 'custom') ? (bulkCustomExpiryInput.value ? new Date(bulkCustomExpiryInput.value) : null) : getPresetExpiryDate(choice);
                    if (!newExpiryDate) return alert('有効期限を指定してください。');
                    selectedIds.forEach(id => batch.update(doc(db, 'members', id), { expiryDate: Timestamp.fromDate(newExpiryDate), isExpired: false }));
                }
                await batch.commit();
        ...
        function renderMemberList() {
            ...
            displayedMembers = displayedMembers.filter(member => {
                ...
                if (searchTerm) {
                    ...
                    return name.includes(searchTerm) || furigana.includes(searchTerm) || project.includes(searchTerm) || grade.includes(searchTerm) || age.includes(searchTerm);
                }
                return true;
            });
            ...
            displayedMembers.forEach(member => {
                ...
                const rolesText = (member.roles && member.roles.length > 0) ? member.roles.join(', ') : 'なし';
    
                const editRolesHtml = ROLES_LIST.map(role => {
                    const isChecked = (member.roles || []).includes(role) ? 'checked' : '';
                    return `<label><input type="checkbox" name="edit-role-${member.id}" value="${role}" ${isChecked}> ${role}</label>`;
                }).join('');
                ...
                const html = `
                    <div class="${memberClass}" id="member-item-${member.id}">
                        ...
                        <div class="member-details">
                            ...
                            <p><strong>役職:</strong> ${rolesText}</p>
                        ...
                        <div class="edit-view" style="display:none;">
                            <div class="edit-form">
                                ...
                                <div class="form-group full-width">
                                    <label>役職</label>
                                    <div class="checkbox-group">${editRolesHtml}</div>
                                </div>
    ...
    function initExcelHandlers() {
        ...
        downloadTemplateButton.addEventListener('click', () => {
            const exampleRow = { '名前(必須)': '電通 太郎', 'フリガナ(必須)': 'デンツウ タロウ' };
            ...
            try {
                ...
                reader.onload = async (event) => {
                    try {
                        ...
                        console.error("Excelファイル読み込みエラー:", readError);
                        alert('ファイルの読み込み中にエラーが発生しました。ファイル形式が正しいか確認してください。');
                    } finally {
    ...
    async function loadActivityLogs() {
        ...
        const docsPerPage = parseInt(document.getElementById('log-page-size-select').value, 10);
